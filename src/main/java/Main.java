...
public class Main {//6 ст join поочередная работа потоков, сперва создается массив потом потоки по нему пробегают
    //создать два потока , первый создает массив[10лям] и заполняет его 1 находиться он в майне,второй поток- создать класс с наследование потока и в нем метод деления масива по полам и все осатальное, так же создать мониторы, для поочередного выполнения дейсвий и замерять время выполнения работы
    ...
    private static int size = 10;//_000_000;
